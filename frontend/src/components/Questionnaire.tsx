...
export default function Questionnaire({ userId, userName }: QuestionnaireProps) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (response.ok) {
                // Atualizar o localStorage para refletir que as preferências foram completadas
                const userStr = localStorage.getItem('user');
                if (userStr) {
                    const userData = JSON.parse(userStr);
                    userData.preferences_completed = true;
                    localStorage.setItem('user', JSON.stringify(userData));
                    // Disparar evento de storage para sincronizar outros componentes
    ...
    return (
        <div className="max-w-2xl mx-auto bg-white dark:bg-brand-dark/80 backdrop-blur-xl border border-zinc-100 dark:border-brand-blue/10 rounded-[3rem] p-8 md:p-12 shadow-2xl animate-in fade-in slide-in-from-bottom-8 duration-700">
